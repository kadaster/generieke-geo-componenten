...
describe("CenterDraw", () => {
  ...
  it("updateCenterPoint() should update LineString geometry coordinates", () => {
    ...
    centerDraw["sketchFeature"] = feature;
  ...
  it("updateCenterPoint() should do nothing if sketchGeometry is a Point", () => {
    ...
    expect(feature.getGeometry()!.getCoordinates()).toEqual([50, 50]);
  });
  ...
  it("appendCoordinates() should call initializeSketch if sketchGeometry  is undefined", () => {
    ...
    centerDraw["sketchFeature"] = undefined; // Geen bestaande geometrie
  ...
  it("appendCoordinates() should initialize a Point geometry and set its coordinates", () => {
    ...
    // Roep appendCoordinates aan
    centerDraw.appendCoordinates();
    expect(centerDraw["sketchFeature"]).toBeDefined();
    expect(centerDraw["sketchFeature"]?.getGeometry()).toBeInstanceOf(Point);
    expect(
      (centerDraw["sketchFeature"]?.getGeometry() as Point).getCoordinates()
    ).toEqual([100, 200]);
  ...
  it("appendCoordinates() should add a coordinate to a LineString", () => {
    ...
    expect(
      (centerDraw["sketchFeature"].getGeometry() as LineString).getCoordinates()
    ).toEqual([
      [10, 15],
      [20, 25],
      [100, 200]
  ...
  it("appendCoordinates() should add geometry to a Polygon when geometry is empty", () => {
    ...
    centerDraw["sketchFeature"] = new Feature(new Polygon([]));
    // Roep de methode aan
    centerDraw.appendCoordinates();
    // Verifieer dat het nieuwe punt vóór het sluitpunt in de ring is toegevoegd
    expect(
      (
        centerDraw["sketchFeature"]?.getGeometry() as Polygon
      ).getCoordinates()[0]
    ).toEqual([
      [100, 200],
      [100, 200],
  ...
  it("appendCoordinates() should add a coordinate to a Polygon", () => {
    ...
    // Verifieer dat het nieuwe punt vóór het sluitpunt in de ring is toegevoegd
    expect(
      (centerDraw["sketchFeature"]?.getGeometry() as Polygon).getCoordinates()
    ).toEqual([
      [
        [10, 20],
        [20, 30],
  ...
  it("removeLastPoint() should remove last point of Linestring", () => {
    ...
    expect(
      (
        centerDraw["sketchFeature"]?.getGeometry() as LineString
      ).getCoordinates()
    ).toEqual([
      [10, 20],
      [100, 200] // Middelpunt toegevoegd
  ...
  it("finishDrawing() should add a Point  and finish the drawing", () => {
    centerDraw["sketchFeature"] = new Feature(new Point([10, 20]));
    centerDraw.finishDrawing();
    expect(centerDraw["targetSource"]?.getFeatures().length).toBe(1);
  });

  it("abortDrawing() should reset sketchGeometrie and sketchFeature", () => {
    centerDraw["sketchFeature"] = new Feature(
      new LineString([
        [10, 20],
        [30, 40]
