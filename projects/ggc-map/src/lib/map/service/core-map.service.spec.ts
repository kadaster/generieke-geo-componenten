...
describe("CoreMapService", () => {
  ...
  describe("create, get and destroyMap", () => {
    ...
    it(
      ...
      () => {
        ...
        const newMap: Map = coreMapService.createAndGetMap(
          myMap,
          3,
          19
        ) as unknown as Map;
    ...
    it("getMap, when called with not existing mapIndex, should return a map", () => {
      const myMap = "testMap";
    ...
    it("checkMapIndex, when called with existing mapIndex, should true", () => {
      expect(coreMapService["olMaps"].size).toBe(0);
    ...
    it("checkMapIndex, when called with not existing mapIndex, should false", () => {
      expect(coreMapService["olMaps"].size).toBe(0);
  ...
  describe("highlightLayer", () => {
    it("addFeaturesToHighlightLayer, when called with 1 feature, features should be 1.", () => {
      // before spyOn mapService, call createAndGetMap() to make sure a map (with default mapIndex) created
      coreMapService.createAndGetMap();
      expect(coreMapService["extraLayersMap"].size).toBe(2);
    ...
    it("clearHighlightLayer, when map does not exist, a new MapComponentEvent of type UNSUCCESSFUL shoud be returned.", () => {
      const checkMapIndexSpy = vi.spyOn(coreMapService, "checkMapIndex");

      const mapComponentEvent = coreMapService.clearHighlightLayer("onbekend");
    ...
    it("clearHighlightLayer, when clear() is called number of features should be zero.", () => {
    ...
    it("when changeHighlightLayerStyle() is called it should change the default to the style given as parameter", () => {
      coreMapService.createAndGetMap();
      ...
      expect(defaultStyle.getFill()?.getColor()).toBe("rgba(0, 115, 149, 0.5)");
  ...
  describe("selectionLayer", () => {
    ...
    it("addFeaturesToSelectionLayer, when map does not exist, a new MapComponentEvent of type UNSUCCESSFUL shoud be returned.", () => {
      const checkMapIndexSpy = vi.spyOn(coreMapService, "checkMapIndex");
    ...
    it("clearSelectionLayer, when clear() is called number of features should be zero.", () => {
      // first, call createAndGetMap() to make sure a map (with default mapIndex) created
      coreMapService.createAndGetMap();
      const vectorSource =
        coreMapService["getSelectionLayerSource"](DEFAULT_MAPINDEX);
    ...
    it("clearSelectionLayer, when map does not exist, a new MapComponentEvent of type UNSUCCESSFUL shoud be returned.", () => {
      const checkMapIndexSpy = vi.spyOn(coreMapService, "checkMapIndex");

      const mapComponentEvent = coreMapService.clearSelectionLayer("onbekend");
    ...
    it("when changeSelectionLayerStyle() is called it should change the default to the style given as parameter", () => {
      coreMapService.createAndGetMap();
      ...
      expect(defaultStyle.getFill()?.getColor()).toBe("rgba(255,255,255,0.5)");
    ...
    it("should only create geolocation layer once and return the same layer when called again", () => {
      ...
      const sameLayer = coreMapService.getExtraLayer("geolocation", "testMap");
