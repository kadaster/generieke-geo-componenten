...
function getGenericContent() {
  ...
  content = content.replace(" De documentatie in GGC Home (onder `/projects/ggc-home`) is gelicentieerd onder [Creative Commons Attribution Share Alike 4.0 International (`CC-BY-SA-4.0`)](https://creativecommons.org/licenses/by-sa/4.0/deed.nl).", "");
  content = content.replace("CONTRIBUTING.md", "https://github.com/kadaster/generieke-geo-componenten/tree/main?tab=contributing-ov-file");
...
function processProjects() {
  ...
  projects.forEach(project => {
    ...
    if (fs.existsSync(projectReadmePath)) {
      ...
      console.log(`ℹ️ Geen project README voor ${project}, alleen generieke content`);
    }

    const title = `# Generieke Geo Componenten - ${project}`;

    // Titel weghalen, wordt later aan het begin toegevoegd
    let modifiedGeneric = genericContent.replace(
      /^# .*/m,
      ""
    );
    ...
    if (project === "ggc-home") {
      modifiedGeneric = modifiedGeneric.replace("European Union Public License (EUPL) v1.2", "CC BY-SA 4.0 licentie");
    }

    const finalContent = projectSpecific ? `${title}\n${projectSpecific}\n\n---\n${modifiedGeneric}` : `${title}${modifiedGeneric}`;
